...
def apply_contamination(sequence, contaminant_sequence, percentage):
    """
    Mixes a percentage of the contaminant sequence into the original sequence.
    percentage: float, 0.0 to 1.0 (e.g. 0.1 for 10% contamination)
    The length of the added contaminant is percentage * length(sequence).
    """
    if percentage == 0:
        return sequence
    ...
    # If contaminant is shorter than target length, wrap around or repeat
    ...
    # Take a random chunk or just the first N elements? 
    # Random chunk is better for robustness
...
def load_all_chromosomes(data_dir, individual, chr_order):
    """
    Load chromosomes for a given individual into a dict {chrom_name: sequence_tensor}.
    Prefers hap1 when multiple haplotypes exist; falls back to any available haplotype.
    For individuals with empty haplotype (chm13, hg38) all keys are included directly.
    """
    path = os.path.join(data_dir, f"{individual}.pt")
    raw = torch.load(path)

    # Group keys by chromosome: {chrom: [(hap, tensor), ...]}
    by_chrom = {}
    for (ind, hap, chrom), v in raw.items():
    ...
    for chrom, entries in by_chrom.items():
        # Prefer hap1, then hap2, then whatever is first
        hap_order = ['hap1', 'hap2', '']
        chosen = None
...
def run_for_individual(individual, data_dir, loader, chr_order, contamination_levels, n_sims, palette, output_dir, chm13_seqs):
    ...
    for idx, target_chrom in enumerate(available):
        ...
        # Reference is CHM13 for this chromosome
        ...
        ax.plot(contamination_levels * 100, metric_values,
                color=color, alpha=0.7, label=target_chrom, linewidth=1.5)
        all_results.append({'chromosome': target_chrom, 'scores': metric_values})

    if all_results:
        avg_scores = np.mean([r['scores'] for r in all_results], axis=0)
        ax.plot(contamination_levels * 100, avg_scores,
                color='black', linewidth=3, linestyle='--', label='Average')

    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel("Contamination added (% of original chromosome length)", fontsize=12)
    ax.set_ylabel("KL", fontsize=12)
    plt.xticks(ticks=[0.1, 0.5, 1, 5, 10, 50, 100, 300], labels=['0.1%', '0.5%', '1%', '5%', '10%', '50%', '100%', '300%'], fontsize=10)
...
def main():
    ...
    # All individuals available in data dir
    ...
    for individual in individuals:
        run_for_individual(individual, data_dir, loader, chr_order,
                           contamination_levels, n_sims, palette, output_dir, chm13_seqs)
